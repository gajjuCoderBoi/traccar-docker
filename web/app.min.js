...
var to=new Date(this.toDate.getFullYear(),this.toDate.getMonth(),this.toDate.getDate(),this.toTime.getHours(),this.toTime.getMinutes(),this.toTime.getSeconds(),this.toTime.getMilliseconds());this.reportProgress=!0;this.updateButtons();'showButton'===button.reference?('chart'===reportType?(button=this.getChart().getStore(),this.getChart().setSeries([])):button=this.getGrid().getStore(),button.showMarkers=this.showMarkers,button.load({scope:this,callback:function(){this.reportProgress=!1;this.updateButtons()},
params:{deviceId:this.deviceId,groupId:this.groupId,type:this.eventType,from:from.toISOString(),to:to.toISOString()}})):(reportType=this.getGrid().getStore().getProxy().url,this.excelReport(reportType,{deviceId:this.deviceId,groupId:this.groupId,type:this.eventType,from:Ext.Date.format(from,'c'),to:Ext.Date.format(to,'c'),mail:'emailButton'===button.reference}))}},onClearClick:function(){var reportType=this.lookupReference('reportTypeField').getValue();this.clearReport(reportType)},clearReport:function(reportType){this.getGrid().getStore().removeAll();
'trips'!==reportType&&'events'!==reportType&&'stops'!==reportType||Ext.getStore('ReportRoute').removeAll();'chart'===reportType&&this.getChart().getStore().removeAll()},onSelectionChange:function(selection,selected){0<selected.length&&(selection=selected[0],this.fireEvent('selectreport',selection,!0),selection instanceof Traccar.model.ReportTrip&&this.selectTrip(selection),(selection instanceof Traccar.model.Event||selection instanceof Traccar.model.ReportStop)&&this.selectPositionRelated(selection))},
selectReport:function(object){var reportType=this.lookupReference('reportTypeField').getValue();if(object instanceof Traccar.model.Position)if('route'===reportType)this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object);else if('events'===reportType||'stops'===reportType)object=this.getGrid().getStore().findRecord('positionId',object.get('id'),0,!1,!0,!0),this.getGrid().getSelectionModel().select([object],!1,!0),this.getGrid().getView().focusRow(object)},
deselectReport:function(object){object&&this.deselectFeature()},deselectFeature:function(){'trips'!==this.lookupReference('reportTypeField').getValue()&&this.getGrid().getSelectionModel().deselectAll()},selectTrip:function(trip){var from=new Date(trip.get('startTime'));var to=new Date(trip.get('endTime'));Ext.getStore('ReportRoute').removeAll();Ext.getStore('ReportRoute').showMarkers=this.showMarkers;Ext.getStore('ReportRoute').load({params:{deviceId:trip.get('deviceId'),from:from.toISOString(),to:to.toISOString()}})},
selectPositionRelated:function(report){report.get('positionId')&&(report=Ext.getStore('ReportRoute').getById(report.get('positionId')))&&this.fireEvent('selectreport',report,!0)},loadRelatedPositions:function(store,data){var positionIds=[];Ext.getStore('ReportRoute').removeAll();for(store=0;store<data.length;store++){var reportObject=data[store];reportObject.get('positionId')&&positionIds.push(reportObject.get('positionId'))}0<positionIds.length&&Ext.getStore('Positions').load({params:{id:positionIds},
...
this.tripsColumns),this.getView().getLayout().setActiveItem('grid')):'stops'===newValue?(this.getGrid().reconfigure('ReportStops',this.stopsColumns),this.getView().getLayout().setActiveItem('grid')):'chart'===newValue&&this.getView().getLayout().setActiveItem('chart');this.updateButtons()},onGeocode:function(positionId){var position=Ext.getStore('ReportRoute').getById(positionId);position&&!position.get('address')&&Ext.Ajax.request({scope:this,method:'GET',url:'api/server/geocode',params:{latitude:position.get('latitude'),
longitude:position.get('longitude')},success:function(response){position.set('address',response.responseText);position.commit();this.fireEvent('selectReport',position)},failure:function(response){Traccar.app.showError(response)}})},routeColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.positionValid,dataIndex:'valid',renderer:Traccar.AttributeFormatter.getFormatter('valid')},{text:Strings.positionFixTime,dataIndex:'fixTime',
xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('fixTime')},{text:Strings.positionLatitude,dataIndex:'latitude',renderer:Traccar.AttributeFormatter.getFormatter('latitude')},{text:Strings.positionLongitude,dataIndex:'longitude',renderer:Traccar.AttributeFormatter.getFormatter('longitude')},{text:Strings.positionAltitude,dataIndex:'altitude',renderer:Traccar.AttributeFormatter.getFormatter('altitude')},{text:Strings.positionSpeed,dataIndex:'speed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},
...
{text:Strings.sharedType,dataIndex:'type',renderer:function(value){return Traccar.app.getEventString(value)}},{text:Strings.sharedGeofence,dataIndex:'geofenceId',renderer:Traccar.AttributeFormatter.getFormatter('geofenceId')},{text:Strings.sharedMaintenance,dataIndex:'maintenanceId',renderer:Traccar.AttributeFormatter.getFormatter('maintenanceId')}],summaryColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.sharedDistance,
dataIndex:'distance',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportStartOdometer,dataIndex:'startOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportEndOdometer,dataIndex:'endOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportAverageSpeed,dataIndex:'averageSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},{text:Strings.reportMaximumSpeed,dataIndex:'maxSpeed',renderer:Traccar.AttributeFormatter.getFormatter('speed')},
{text:Strings.reportEngineHours,dataIndex:'engineHours',renderer:Traccar.AttributeFormatter.getFormatter('duration')},{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')}],tripsColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.reportStartTime,dataIndex:'startTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('startTime')},{text:Strings.reportStartOdometer,
dataIndex:'startOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportStartAddress,dataIndex:'startAddress',renderer:Traccar.AttributeFormatter.getFormatter('address')},{text:Strings.reportEndTime,dataIndex:'endTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('endTime')},{text:Strings.reportEndOdometer,dataIndex:'endOdometer',renderer:Traccar.AttributeFormatter.getFormatter('distance')},{text:Strings.reportEndAddress,dataIndex:'endAddress',
...
{text:Strings.reportSpentFuel,dataIndex:'spentFuel',renderer:Traccar.AttributeFormatter.getFormatter('spentFuel')},{text:Strings.sharedDriver,dataIndex:'driverUniqueId',renderer:Traccar.AttributeFormatter.getFormatter('driverUniqueId')}],stopsColumns:[{text:Strings.reportDeviceName,dataIndex:'deviceId',renderer:Traccar.AttributeFormatter.getFormatter('deviceId')},{text:Strings.reportStartTime,dataIndex:'startTime',xtype:'datecolumn',renderer:Traccar.AttributeFormatter.getFormatter('startTime')},{text:Strings.positionOdometer,
...
Ext.define('Traccar.view.map.BaseMap',{extend:Ext.panel.Panel,xtype:'baseMapView',layout:'fit',getMap:function(){return this.map},getMapView:function(){return this.mapView},initMap:function(){var self=this;var server=Traccar.app.getServer();var type=Traccar.app.getPreference('map',null);var bingKey=server.get('bingKey');switch(type){case 'custom':server=new ol.layer.Tile({source:new ol.source.XYZ({url:Ext.String.htmlDecode(server.get('mapUrl')),attributions:''})});break;case 'customArcgis':server=
new ol.layer.Tile({source:new ol.source.TileArcGISRest({url:Ext.String.htmlDecode(server.get('mapUrl'))})});break;case 'bingRoad':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'Road'})});break;case 'bingAerial':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'Aerial'})});break;case 'bingHybrid':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'AerialWithLabels'})});break;case 'carto':server=new ol.layer.Tile({source:new ol.source.XYZ({url:'https://cartodb-basemaps-{a-d}.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png',
attributions:'\x26copy; \x3ca href\x3d"http://www.openstreetmap.org/copyright"\x3eOpenStreetMap\x3c/a\x3e contributors, \x26copy; \x3ca href\x3d"https://carto.com/attributions"\x3eCARTO\x3c/a\x3e'})});break;case 'baidu':server=new ol.layer.Tile({source:new ol.source.XYZ({projection:'BD-MC',tileUrlFunction:function(tileCoord){var z=tileCoord[0],x=tileCoord[1];tileCoord=tileCoord[2];var index=((x<<z)+tileCoord)%5;0>x&&(x='M'+-x);0>tileCoord&&(tileCoord='M'+-tileCoord);return'https://online{}.map.bdimg.com/onlinelabel/?qt\x3dtile\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26styles\x3dpl'.replace('{}',
0>index?index+5:index).replace('{x}',x).replace('{y}',tileCoord).replace('{z}',z)},tileGrid:new ol.tilegrid.TileGrid({extent:ol.proj.transformExtent([-180,-74,180,74],'EPSG:4326','BD-MC'),origin:[0,0],minZoom:3,resolutions:[262144,131072,65536,32768,16384,8192,4096,2048,1024,512,256,128,64,32,16,8,4,2,1,.5]}),attributions:'\x26copy; \x3ca href\x3d"https://map.baidu.com/"\x3eBaidu\x3c/a\x3e'})});break;case 'yandexMap':server=new ol.layer.Tile({source:new ol.source.XYZ({url:'https://vec0{1-4}.maps.yandex.net/tiles?l\x3dmap\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}',
projection:'EPSG:3395',attributions:'\x26copy; \x3ca href\x3d"https://yandex.com/maps/"\x3eYandex\x3c/a\x3e'})});break;case 'yandexSat':server=new ol.layer.Tile({source:new ol.source.XYZ({url:'https://sat0{1-4}.maps.yandex.net/tiles?l\x3dsat\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}',projection:'EPSG:3395',attributions:'\x26copy; \x3ca href\x3d"https://yandex.com/maps/"\x3eYandex\x3c/a\x3e'})});break;case 'wikimedia':server=new ol.layer.Tile({source:new ol.source.OSM({url:'https://maps.wikimedia.org/osm-intl/{z}/{x}/{y}.png'})});
break;default:server=new ol.layer.Tile({source:new ol.source.OSM({})})}type=Traccar.app.getPreference('latitude',Traccar.Style.mapDefaultLat);bingKey=Traccar.app.getPreference('longitude',Traccar.Style.mapDefaultLon);var zoom=Traccar.app.getPreference('zoom',Traccar.Style.mapDefaultZoom);var maxZoom=Traccar.app.getAttributePreference('web.maxZoom',Traccar.Style.mapMaxZoom);this.mapView=new ol.View({center:ol.proj.fromLonLat([bingKey,type]),zoom:zoom,maxZoom:maxZoom});this.map=new ol.Map({target:this.body.dom.id,
layers:[server],view:this.mapView});(server=Traccar.app.getPreference('poiLayer',null))&&this.map.addLayer(new ol.layer.Vector({source:new ol.source.Vector({url:server,format:new ol.format.KML})}));this.body.dom.tabIndex=0;switch(Traccar.app.getAttributePreference('distanceUnit','km')){case 'mi':this.map.addControl(new ol.control.ScaleLine({units:'us'}));break;case 'nmi':this.map.addControl(new ol.control.ScaleLine({units:'nautical'}));break;default:this.map.addControl(new ol.control.ScaleLine)}var target=
this.map.getTarget();'string'===typeof target&&(target=Ext.get(target).dom);this.map.on('pointermove',function(e){this.forEachFeatureAtPixel(e.pixel,function(){return!0})?target.style.cursor='pointer':target.style.cursor=''});this.map.on('click',function(e){var features=self.map.getFeaturesAtPixel(e.pixel,{layerFilter:function(layer){return!layer.get('name')}});if(features)for(e=0;e<features.length;e++)self.fireEvent('selectfeature',features[e]);else self.fireEvent('deselectfeature')})},listeners:{afterrender:function(){this.initMap()},
resize:function(){this.map.updateSize()}}},function(){var projection;proj4.defs('BD-MC','+proj\x3dmerc +lon_0\x3d0 +units\x3dm +ellps\x3dclrk66 +no_defs');proj4.defs('EPSG:3395','+proj\x3dmerc +lon_0\x3d0 +k\x3d1 +x_0\x3d0 +y_0\x3d0 +datum\x3dWGS84 +units\x3dm +no_defs');ol.proj.proj4.register(proj4);(projection=ol.proj.get('EPSG:3395'))&&projection.setExtent([-2.0037508342789244E7,-2.0037508342789244E7,2.0037508342789244E7,2.0037508342789244E7])});
...
markersSource.removeFeature(this.liveRoutes[deviceId]),delete this.liveRoutes[deviceId])}},animateMarker:function(marker$jscomp$0,geometry,course){var start=marker$jscomp$0.getGeometry().getCoordinates();geometry=geometry.getCoordinates();start=new ol.geom.LineString([start,geometry]);var duration=Traccar.Style.mapAnimateMarkerDuration;var timeout=Traccar.Style.mapAnimateMarkerTimeout;var self=this;var updatePosition=function(position,marker){var coordinate=marker.get('line').getCoordinateAt(position/
(duration/timeout));var style=marker.getStyle();marker.setGeometry(new ol.geom.Point(coordinate));position<duration/timeout?setTimeout(updatePosition,timeout,position+1,marker):(style.getImage().angle!==marker.get('nextCourse')&&self.rotateMarker(style,marker.get('nextCourse')),marker.set('animating',!1))};marker$jscomp$0.set('line',start);marker$jscomp$0.set('nextCourse',course);marker$jscomp$0.get('animating')||(marker$jscomp$0.set('animating',!0),updatePosition(1,marker$jscomp$0))},updateLatest:function(store,
data){var device;Ext.isArray(data)||(data=[data]);var deviceStore=Ext.getStore('Devices');for(store=0;store<data.length;store++){var position=data[store];if(device=deviceStore.getById(position.get('deviceId')))this.updateAccuracy(position,device),this.updateLatestMarker(position,device),this.updateLiveRoute(position,device)}},updateAccuracy:function(position,device){var feature=this.accuracyCircles[position.get('deviceId')];if(position.get('accuracy')){var center=ol.proj.fromLonLat([position.get('longitude'),
position.get('latitude')]);var radius=Ext.getStore('DistanceUnits').convertValue(position.get('accuracy'),Traccar.app.getAttributePreference('distanceUnit'),!0);feature?(feature.getGeometry().setCenter(center),feature.getGeometry().setRadius(radius)):(feature=new ol.Feature(new ol.geom.Circle(center,radius)),feature.setStyle(this.getAreaStyle(null,Traccar.Style.mapAccuracyColor)),feature.setId(position.get('deviceId')),this.accuracyCircles[position.get('deviceId')]=feature,this.isDeviceVisible(device)&&
this.getView().getAccuracySource().addFeature(feature))}else feature&&this.getView().getAccuracySource().getFeatureById(feature.getId())&&this.getView().getAccuracySource().removeFeature(feature),delete this.accuracyCircles[position.get('deviceId')]},updateLatestMarker:function(position,device){var geometry=new ol.geom.Point(ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')]));var deviceId=position.get('deviceId');if(deviceId in this.latestMarkers){var marker=this.latestMarkers[deviceId];
this.animateMarker(marker,geometry,position.get('course'))}else marker=new ol.Feature(geometry),marker.set('record',device),position=this.getLatestMarker(this.getDeviceColor(device),position.get('course'),device.get('category')),position.getText().setText(Ext.String.htmlDecode(device.get('name'))),marker.setStyle(position),marker.setId(device.get('id')),this.latestMarkers[deviceId]=marker,this.isDeviceVisible(device)&&this.getView().getMarkersSource().addFeature(marker);marker===this.selectedMarker&&
this.lookupReference('deviceFollowButton').pressed&&this.getView().getMapView().setCenter(marker.getGeometry().getCoordinates())},updateLiveRoute:function(position,device){var deviceId=position.get('deviceId');if(deviceId in this.liveRoutes){device=this.liveRoutes[deviceId].getGeometry().getCoordinates();var lastLiveCoordinates=device[device.length-1];position=ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')]);if(lastLiveCoordinates[0]!==position[0]||lastLiveCoordinates[1]!==
position[1])device.length>=this.liveRouteLength&&device.shift(),device.push(position),this.liveRoutes[deviceId].getGeometry().setCoordinates(device)}else position=new ol.Feature({geometry:new ol.geom.LineString([ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')])])}),position.setStyle(this.getRouteStyle(deviceId)),position.setId(deviceId),this.liveRoutes[deviceId]=position,this.isDeviceVisible(device)&&this.getView().getMarkersSource().addFeature(position)},loadReport:function(store,
data){if(data){this.addReportMarkers(store,data);var routeSource=this.getView().getRouteSource();this.reportRoute=[];for(store=0;store<data.length;store++){var position=data[store];position=ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')]);if(0===store||data[store].get('deviceId')!==data[store-1].get('deviceId'))this.reportRoute.push(new ol.Feature({geometry:new ol.geom.LineString([])})),this.reportRoute[this.reportRoute.length-1].setStyle(this.getRouteStyle(data[store].get('deviceId'))),
routeSource.addFeature(this.reportRoute[this.reportRoute.length-1]);this.reportRoute[this.reportRoute.length-1].getGeometry().appendCoordinate(position)}}},addReportMarker:function(position){var geometry=ol.proj.fromLonLat([position.get('longitude'),position.get('latitude')]);geometry=new ol.geom.Point(geometry);geometry=new ol.Feature(geometry);geometry.set('record',position);position=this.getReportMarker(position.get('deviceId'),position.get('course'));geometry.setStyle(position);this.getView().getMarkersSource().addFeature(geometry);
return geometry},addReportMarkers:function(store,data){var i;this.clearReport();for(i=0;i<data.length;i++)store.showMarkers&&(this.reportMarkers[data[i].get('id')]=this.addReportMarker(data[i]));this.zoomToAllPositions(data)},clearReport:function(){var key,i;var reportSource=this.getView().getRouteSource();if(this.reportRoute){for(i=0;i<this.reportRoute.length;i++)reportSource.removeFeature(this.reportRoute[i]);this.reportRoute=null}if(this.reportMarkers){i=this.getView().getMarkersSource();for(key in this.reportMarkers)this.reportMarkers.hasOwnProperty(key)&&
i.removeFeature(this.reportMarkers[key]);this.reportMarkers={}}this.selectedMarker&&!this.selectedMarker.get('event')&&this.selectedMarker.get('record')instanceof Traccar.model.Position&&(this.selectedMarker=null)},clearEvent:function(){this.selectedMarker&&this.selectedMarker.get('event')&&this.selectMarker(null,!1)},getRouteStyle:function(deviceId){return new ol.style.Style({stroke:new ol.style.Stroke({color:Traccar.app.getReportColor(deviceId),width:Traccar.Style.mapRouteWidth})})},getMarkerStyle:function(zoom,
color,angle,category){zoom=Traccar.DeviceImages.getImageIcon(color,zoom,angle,category);return new ol.style.Style({image:zoom,text:new ol.style.Text({textBaseline:'bottom',fill:new ol.style.Fill({color:Traccar.Style.mapTextColor}),stroke:new ol.style.Stroke({color:Traccar.Style.mapTextStrokeColor,width:Traccar.Style.mapTextStrokeWidth}),offsetY:-zoom.getSize()[1]/2-Traccar.Style.mapTextOffset,font:Traccar.Style.mapTextFont})})},getLatestMarker:function(color,angle,category){return this.getMarkerStyle(!1,
color,angle,category)},getReportMarker:function(deviceId,angle){return this.getMarkerStyle(!1,Traccar.app.getReportColor(deviceId),angle,'arrow')},resizeMarker:function(style,zoom){zoom=Traccar.DeviceImages.getImageIcon(style.getImage().fill,zoom,style.getImage().angle,style.getImage().category);var text=style.getText();text.setOffsetY(-zoom.getSize()[1]/2-Traccar.Style.mapTextOffset);style.setText(text);style.setImage(zoom)},rotateMarker:function(style,angle){style.setImage(Traccar.DeviceImages.getImageIcon(style.getImage().fill,
style.getImage().zoom,angle,style.getImage().category))},updateDeviceMarker:function(style,color,category){color=Traccar.DeviceImages.getImageIcon(color,style.getImage().zoom,style.getImage().angle,category);category=style.getText();category.setOffsetY(-color.getSize()[1]/2-Traccar.Style.mapTextOffset);style.setText(category);style.setImage(color)},selectMarker:function(marker,center){this.selectedMarker&&(this.selectedMarker.get('event')?this.getView().getMarkersSource().removeFeature(this.selectedMarker):
...
center)):this.selectedMarker&&this.selectMarker(null,!1)},selectEvent:function(position){position?(position=this.addReportMarker(position),position.set('event',!0),this.selectMarker(position,!0)):this.selectedMarker&&this.selectedMarker.get('event')&&this.selectMarker(null,!1)},selectFeature:function(feature){(feature=feature.get('record'))&&(feature instanceof Traccar.model.Device?this.fireEvent('selectdevice',feature,!1):this.fireEvent('selectreport',feature,!1))},deselectFeature:function(){this.deselectDevice();
...
point&&this.getView().getMapView().fit(new ol.geom.Point(point))},updateVisibleDevices:function(store,data){Ext.isArray(data)||(data=[data]);for(store=0;store<data.length;store++){var device=data[store];device.get('id')in this.latestMarkers&&this.updateDeviceVisibility(device)}},isDeviceVisible:function(device){return Ext.getStore('VisibleDevices').contains(device)},updateDeviceVisibility:function(device){var deviceId=device.get('id');var marker=this.latestMarkers[deviceId];var accuracy=this.accuracyCircles[deviceId];
...
feature.setStyle(this.getAreaStyle(Ext.String.htmlDecode(geofence.get('name')),geofence.get('attributes')?geofence.get('attributes').color:null));this.getView().getGeofencesSource().addFeature(feature);return!0},this)},zoomToAllDevices:function(){this.zoomToAllPositions(Ext.getStore('LatestPositions').getData().items)}});
...
inputValue:!0,uncheckedValue:!1,name:'forceSettings',fieldLabel:Strings.serverForceSettings},{xtype:'clearableComboBox',name:'coordinateFormat',fieldLabel:Strings.settingsCoordinateFormat,store:'CoordinateFormats',displayField:'name',valueField:'key'},{xtype:'unescapedTextField',name:'poiLayer',fieldLabel:Strings.mapPoiLayer}]},{xtype:'fieldset',title:Strings.sharedPermissions,collapsible:!0,collapsed:!0,items:[{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'registration',fieldLabel:Strings.serverRegistration},
{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'readonly',fieldLabel:Strings.serverReadonly},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'deviceReadonly',fieldLabel:Strings.userDeviceReadonly},{xtype:'checkboxfield',inputValue:!0,uncheckedValue:!1,name:'limitCommands',fieldLabel:Strings.userLimitCommands}]}]},buttons:[{text:Strings.sharedAttributes,handler:'showAttributesView'},{glyph:'xf041@FontAwesome',minWidth:0,handler:'getMapState',tooltip:Strings.sharedGetMapState,
tooltipType:'title'},{xtype:'tbfill'},{glyph:'xf00c@FontAwesome',tooltip:Strings.sharedSave,tooltipType:'title',minWidth:0,handler:'onSaveClick'},{glyph:'xf00d@FontAwesome',tooltip:Strings.sharedCancel,tooltipType:'title',minWidth:0,handler:'closeView'}]});
...
Ext.define('Traccar.view.map.GeofenceMapController',{extend:Ext.app.ViewController,alias:'controller.geofenceMap',config:{listen:{controller:{'*':{mapstate:'setMapState'}}}},onSaveClick:function(button){if(0<this.getView().getFeatures().getLength()){var geometry=this.getView().getFeatures().pop().getGeometry();var projection=this.getView().getMapView().getProjection();this.fireEvent('savearea',Traccar.GeofenceConverter.geometryToWkt(projection,geometry));button.up('window').close()}},onCancelClick:function(button){button.up('window').close()},
onTypeSelect:function(combo){this.getView().removeInteraction();this.getView().addInteraction(combo.getValue())},setMapState:function(lat,lon,zoom){this.getView().getMapView().setCenter(ol.proj.fromLonLat([lon,lat]));this.getView().getMapView().setZoom(zoom)}});
...
getFeatures:function(){return this.features},initMap:function(){this.callParent();var map=this.map;this.features=new ol.Collection;if(this.area){var geometry=Traccar.GeofenceConverter.wktToGeometry(this.mapView,this.area);this.features.push(new ol.Feature(geometry));this.mapView.fit(geometry)}else this.controller.fireEvent('mapstaterequest');geometry=ol.color.asArray(Traccar.Style.mapGeofenceColor);geometry[3]=Traccar.Style.mapGeofenceOverlayOpacity;(new ol.layer.Vector({source:new ol.source.Vector({features:this.features}),
...
addInteraction:function(type){var self=this;this.draw=new ol.interaction.Draw({features:this.features,type:type});this.draw.on('drawstart',function(){self.features.clear()});this.map.addInteraction(this.draw)},removeInteraction:function(){this.draw&&(this.map.removeInteraction(this.draw),this.draw=null)}});
...
persist:!1}]);'number'===attribute.get('valueType')&&(chartTypesStore.add({key:'attribute.'+attribute.get('key'),name:attribute.get('name')}),maintenanceTypesStore.add(attribute))}},calculateAttribute:function(data){data=data.attributes[this.attributeKey];return void 0!==data?Traccar.AttributeFormatter.getAttributeConverter(this.attributeKey)(data):data},onLaunch:function(){Ext.Ajax.request({scope:this,url:'api/server',callback:this.onServerReturn})},onServerReturn:function(options,success,response){options=
{};if(success){Traccar.app.setServer(Ext.decode(response.responseText));if(success=Ext.Object.fromQueryString(window.location.search).token)options.token=success;Ext.Ajax.request({scope:this,url:'api/session',method:'GET',params:options,callback:this.onSessionReturn})}else Traccar.app.showError(response)},onSessionReturn:function(options,success,response){Ext.get('spinner').setVisible(!1);success?(Traccar.app.setUser(Ext.decode(response.responseText)),this.loadApp()):(this.login=Ext.create('widget.login',
{listeners:{scope:this,login:this.onLogin}}),this.login.show())},onLogin:function(){this.login.close();this.loadApp()},loadApp:function(){var updateView;window.webkit&&window.webkit.messageHandlers.appInterface&&window.webkit.messageHandlers.appInterface.postMessage('login');window.appInterface&&window.appInterface.postMessage('login');Ext.getStore('Groups').load();Ext.getStore('Drivers').load();Ext.getStore('Geofences').load();Ext.getStore('Calendars').load();Ext.getStore('Maintenances').load();
Ext.getStore('ComputedAttributes').load();Ext.getStore('AllCommandTypes').load();Ext.getStore('Commands').load();Ext.getStore('AllNotificationTypes').load({callback:function(records,operation,success){operation=Ext.getStore('ReportEventTypes');success&&(operation.add({type:Traccar.store.ReportEventTypes.allEvents,name:Strings.eventAll}),operation.loadData(records,!0))}});Ext.getStore('AllNotificators').load();Ext.getStore('Notifications').load();Ext.getStore('ServerAttributes').loadData(Ext.getStore('CommonDeviceAttributes').getData().items,
...
1);protocol=new WebSocket(protocol+'//'+window.location.host+pathname+'api/socket');protocol.onclose=function(){Traccar.app.showToast(Strings.errorSocket,Strings.errorTitle);Ext.Ajax.request({url:'api/devices',success:function(response){self.updateDevices(Ext.decode(response.responseText))},failure:function(response){401===response.status&&window.location.reload()}});Ext.Ajax.request({url:'api/positions',headers:{Accept:'application/json'},success:function(response){self.updatePositions(Ext.decode(response.responseText))}});
setTimeout(function(){self.asyncUpdate(!1)},Traccar.Style.reconnectTimeout)};protocol.onmessage=function(event){event=Ext.decode(event.data);event.devices&&self.updateDevices(event.devices);event.positions&&(self.updatePositions(event.positions,first),first=!1);event.events&&self.updateEvents(event.events)}},updateDevices:function(array){var i,entity;var store=Ext.getStore('Devices');for(i=0;i<array.length;i++)(entity=store.getById(array[i].id))&&entity.set({status:array[i].status,lastUpdate:array[i].lastUpdate,
geofenceIds:array[i].geofenceIds},{dirty:!1})},updatePositions:function(array,first){var i,entity,device;var store=Ext.getStore('LatestPositions');for(i=0;i<array.length;i++)(entity=store.findRecord('deviceId',array[i].deviceId,0,!1,!1,!0))?entity.set(array[i]):store.add(Ext.create('Traccar.model.Position',array[i])),Ext.getStore('Events').findRecord('positionId',array[i].id,0,!1,!1,!0)&&Ext.getStore('EventPositions').add(Ext.create('Traccar.model.Position',array[i]));first&&((array=Ext.Object.fromQueryString(window.location.search).deviceId)&&
(device=Ext.getStore('VisibleDevices').findRecord('id',array,0,!1,!0,!0))&&this.fireEvent('selectdevice',device,!0),device||this.zoomToAllDevices())},updateEvents:function(array){var i,device;var store=Ext.getStore('Events');for(i=0;i<array.length;i++)store.add(array[i]),(device=Ext.getStore('Devices').getById(array[i].deviceId))?(this.soundPressed()&&this.beep(),Traccar.app.showToast(array[i].text,device.get('name'))):Traccar.app.showToast(array[i].text)},zoomToAllDevices:function(){var lat=Traccar.app.getPreference('latitude',
0);var lon=Traccar.app.getPreference('longitude',0);var zoom=Traccar.app.getPreference('zoom',0);0===lat&&0===lon&&0===zoom&&this.fireEvent('zoomtoalldevices')}});
